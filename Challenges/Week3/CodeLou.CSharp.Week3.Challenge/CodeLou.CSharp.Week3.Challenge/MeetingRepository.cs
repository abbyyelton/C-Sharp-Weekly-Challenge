using System;
using System.Collections.Generic;
using System.Linq;
using Newtonsoft.Json; //Build the project to cause Visual Studio to load this external NuGet package.

namespace CodeLou.CSharp.Week3.Challenge
{
    public class MeetingRepository : ICalendarItemRepository<Meeting>
    {
        //Info: This is a neat type that allows you to lookup items by ID, be careful not to ask for an item that isn't there.
        private readonly Dictionary<int, Meeting> _dictionary;

        public MeetingRepository()
        {
            _dictionary = new Dictionary<int, Meeting>();
        }

        public Meeting Create()
        {
            //Challenge: Can you find a more efficient way to do this?
            var nextAvailableId = 0;
            if (_dictionary.Count > 0)
            {
                nextAvailableId = _dictionary.Keys.Max() + 1;
            }

            var meeting = new Meeting();
            meeting.Id = nextAvailableId;
            _dictionary.Add(nextAvailableId, meeting);

            return meeting;
        }

        //Callenge: Are you finding that you are writing this same code many times? Is there a better way? 
        //Could you use inheritance?
        public Meeting FindById(int id)
        {
            if (id > -1)
            {
                try
                {
                    var meeting = new Meeting();
                    meeting = _dictionary[id];
                    return meeting;
                }
                catch (Exception)
                {
                    return null;
                }
            }
            else
            {
                return null;
            }
        }

        public Meeting Update(Meeting item)
        {
            if (FindById(item.Id) != null)
            {
                _dictionary[item.Id] = item;
                return _dictionary[item.Id];
            }
            else
            {
                return null;
            }
        }

        public void Delete(Meeting item)
        {
            if (item != null)
            {
                _dictionary.Remove(item.Id);
            }
        }

        public IEnumerable<Meeting> FindByDate(DateTime date)
        {
            IEnumerable<Meeting> meetings = new List<Meeting>();

            meetings = from item in _dictionary
                       where (item.Value.StartTime.Date == date.Date)
                       select item.Value;

            return meetings;
        }

        public IEnumerable<Meeting> GetAllItems()
        {
            IEnumerable<Meeting> meetings = new List<Meeting>();

            meetings = _dictionary.Values.ToList();

            return meetings;
        }

        public string ToJson()
        {
            return JsonConvert.SerializeObject(_dictionary, Formatting.Indented);
        }

        public void LoadFromJson(string json)
        {
            var dictionary = JsonConvert.DeserializeObject<Dictionary<int, Meeting>>(json);
            foreach (var item in dictionary)
            {
                //This will add or update an item
                _dictionary[item.Key] = item.Value;
            }
        }

        public void Display()
        {
            Display(_dictionary.Values.ToList());
        }

        public void Display(List<Meeting> list)
        {
            foreach (var item in list)
            {
                Console.WriteLine("Meeting: " + item.StartTime + " - " + item.EndTime + " at " + item.Location
                    + " with " + string.Join(", ", item.Attendees));
            }
        }
    }
}

